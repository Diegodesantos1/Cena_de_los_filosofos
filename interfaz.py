...
class Ventana():
    def __init__(self):
        ...
        tk.Label(self.root, text="Leyenda ", font='System 18 bold', bg=self.fondo).place(x=475,y=0)
        tk.Label(self.root, text=" ",bg="pink").place(x=125,y=40)
        tk.Label(self.root, text="El Filósofo entra a comer", bg=self.fondo).place(x=140,y=40)
        tk.Label(self.root, text=" ",bg="cyan").place(x=125,y=65)
        tk.Label(self.root, text="El filósofo tiene hambre", bg=self.fondo).place(x=140,y=65)
        tk.Label(self.root, text=" ",bg="gold").place(x=125,y=90)
        tk.Label(self.root, text="El filósofo está comiendo", bg=self.fondo).place(x=140,y=90)
        tk.Label(self.root, text=" ",bg="pale green").place(x=350,y=40)
        tk.Label(self.root, text="El filósofo terminó", bg=self.fondo).place(x=365,y=40)
        tk.Label(self.root, text=" ",bg="white").place(x=350,y=65)
        tk.Label(self.root, text="El filósofo está pensando", bg=self.fondo).place(x=365,y=65)
        tk.Label(self.root, text=" ",bg="red").place(x=350,y=90)
        tk.Label(self.root, text="El filósofo se levanta", bg=self.fondo).place(x=365,y=90)
        tk.Label(self.root, text=" ",bg="orange").place(x=585,y=40)
        tk.Label(self.root, text="Tenedor ocupado", bg=self.fondo).place(x=600,y=40)
        tk.Label(self.root, text=" ",bg="light green").place(x=585,y=65)
        tk.Label(self.root, text="Tenedor libre", bg=self.fondo).place(x=600,y=65)
        tk.Label(self.root, text="Número de comidas", font='System 16 bold', bg=self.fondo).place(x=600,y=110)
    def añadirCaja(self):
        angulo=math.pi/N
        for i in range(N):
